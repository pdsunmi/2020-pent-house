...
public class JwtService {
	...
	private Long expireMin = 60L; // 이거 30으로 바꾸면 30분

//	로그인 성공시 사용자 정보를 기반으로 JWTToken을 생성하여 반환.
	public String create(MemberDto memberDto) {
		JwtBuilder jwtBuilder = Jwts.builder(); // 제이슨웹토큰 만듦.
//		JWT Token = Header + Payload + Signature 시그니처는 고유 값.
		
//		Header 설정
		jwtBuilder.setHeaderParam("typ", "JWT"); // 토큰의 타입으로 고정 값.

//		Payload 설정 *** 이게 중요 . 내가 넘겨줘야하는 데이터값.
		jwtBuilder
			.setSubject("로그인토큰") // 토큰의 제목 설정. 토큰에다 이름 붙ㅇ,ㅁ 
			.setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * expireMin)) // 유효기간 설정** 이게 중요. 5분
...
//		signature 설정
		...
		//이거 암호화 HS 256를 이용해서 막 HWEL@##KWE@K#$123143이렇게
